...
//!
//! let hstx_pins = HstxPins.add_bit0_pin(bit0p).add_bit3_pin(bit3p);
//!
//! let mut hstx = Hstx::new(peripherals.HSTX_CTRL, peripherals.HSTX_FIFO, hstx_pins,
//!                          &mut peripherals.RESET);
...
impl<P: ValidHstxPinout> Hstx<Disabled, P> {
    ...
    pub fn configure_clk(&self, div: u8, phase: u8) {
        if phase >= 2 * div {
    ...
    pub fn configure_shifter(&self, shift: u8, n_shifts: u8) {
        self.ctrl
            .csr()
    ...
    /// Enable expander
    pub fn enable_expander(&self) {
        self.ctrl.csr().modify(|_, w| w.expand_en().set_bit());
    }

    /// Disable expander
    pub fn disable_expander(&self) {
...
impl<P: ValidHstxPinout> Hstx<Enabled, P> {
    ...
    pub fn fifo_write(&self, word: u32) {
